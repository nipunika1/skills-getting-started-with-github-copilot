    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants.length > 0) {
          participantsHTML = `<ul style="list-style-type: none; padding-left: 0;">` +
            details.participants.map(p =>
              `<li style="display: flex; align-items: center;">
                <span>${p}</span>
                <span class="delete-participant" title="Unregister" data-activity="${encodeURIComponent(name)}" data-participant="${encodeURIComponent(p)}" style="cursor:pointer; margin-left:8px;">🗑️</span>
              </li>`
            ).join('') +
            `</ul>`;
        } else {
